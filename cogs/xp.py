...
class xp(commands.Cog):
    ...
    async def add_xp(self, userId, guildId, amount):
        ...
        self.client.cursor.execute(
            f"SELECT * FROM usersData WHERE userId = {userId} AND guildId = {guildId}"
        )
        user = self.client.cursor.fetchone()

        if user is None:
            self.client.cursor.execute(
                f"INSERT INTO usersData VALUES({userId},{guildId},{amount});"
            )
            return 0, amount
        else:
            ...
            self.client.cursor.execute(
                f"DELETE FROM usersData WHERE userId = {userId} AND guildId = {guildId};"
            )
            self.client.cursor.execute(
                f"INSERT INTO usersData VALUES({userId},{guildId},{total});"
            )
    ...
    async def get_xp(self, userId, guildId):
        ...
        if user is None:
            ...
            self.client.cursor.execute(
                f"INSERT INTO usersData VALUES({userId},{guildId},0);"
            )
            return 0
